...
CFG = {
    "COMP_DIR":    "/home/user/work/tf-mp/kaggle/birdclef-2026",
    "PSEUDO_DIR":  "/home/user/work/tf-mp/kaggle/working_949",
    "OUT_DIR":     "/home/user/work/tf-mp/kaggle/working_949/train_v2s_out",
...
_MEL_KW = dict(sr=CFG["SR"], n_fft=CFG["N_FFT"], hop_length=CFG["HOP"],
                n_mels=CFG["N_MELS"], fmin=CFG["FMIN"], fmax=CFG["FMAX"], power=2.0)

def wav_to_mel_cpu(wav_np):
    S = _librosa.feature.melspectrogram(y=wav_np, **_MEL_KW)
    M = _librosa.power_to_db(S, top_db=80.0)
    M = (M - M.mean()) / (M.std() + 1e-6)
    return torch.from_numpy(M.astype(np.float32)).unsqueeze(0)
...
if CFG["PSEUDO_DIR"] is not None:
    ...
    if pseudo_path is None:
        cands = list(Path("/home/user/work/tf-mp/kaggle/working_949").rglob("pseudo_labels.parquet"))
        if cands:
            pseudo_path = cands[0]
