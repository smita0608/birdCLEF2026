...
print("Device:", DEVICE, "N_GPUs:", N_GPUS,
      "Backbone:", CFG["BACKBONE"], "SED:", CFG["USE_SED"])
...
def wav_to_melspec_batch(wav_batch):
    with torch.no_grad():
        m = mel_transform(wav_batch)
        m = db_transform(m)
        m = (m - m.mean(dim=(-2,-1), keepdim=True)) / (m.std(dim=(-2,-1), keepdim=True) + 1e-6)
    return m
...
class FocalAudioDataset(Dataset):
    def __init__(self, df, train_dir, is_train=True):
    ...
    def __getitem__(self, i):
        ...
        if COL_SECOND in row and isinstance(row[COL_SECOND], str) and row[COL_SECOND]:
            for s in re.split(r"[,;\s]+", row[COL_SECOND].strip("[]").replace("'", "")):
                if s in CLS2IDX:
                    y[CLS2IDX[s]] = 0.5
        return torch.from_numpy(wav), torch.from_numpy(y)
...
class PseudoSoundscapeDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        return torch.from_numpy(wav.astype(np.float32)), torch.from_numpy(y)
...
def train_one_fold(fold, train_df, val_df, pseudo_df=None):
    ...
    tr_ds = FocalAudioDataset(train_df, BASE / "train_audio", is_train=True)
    va_ds = FocalAudioDataset(val_df,   BASE / "train_audio", is_train=False)
    ...
    if pseudo_df is not None and len(pseudo_df) > 0:
        ps_ds = PseudoSoundscapeDataset(pseudo_df, BASE / "train_soundscapes",
                                        is_train=True)
        pseudo_dl = DataLoader(ps_ds, CFG["BATCH_SIZE"]//2, shuffle=True,
                               num_workers=CFG["NUM_WORKERS"], pin_memory=True, drop_last=True)

    model = BirdCNN(CFG["BACKBONE"], N_CLASSES, use_sed=CFG["USE_SED"]).to(DEVICE)
    if CFG["USE_DP"] and N_GPUS > 1:
    ...
    for epoch in range(CFG["EPOCHS"]):
        ...
        for step, batch in enumerate(iters):
            if not first_batch_logged:
                print(f"  epoch {epoch:02d} first batch loaded ({time.time()-t0:.1f}s)")
            ...
            if pseudo is not None:
                pw, py = pseudo
                wav = torch.cat([wav, pw.to(DEVICE)], 0)
                y   = torch.cat([y,   py.to(DEVICE)], 0)

            mel = wav_to_melspec_batch(wav).unsqueeze(1)
            ...
            sched.step()
            tloss += loss.item() * wav.size(0); n += wav.size(0)
...
for f, (tr, va) in enumerate(folds):
    g_tr, g_va = set(groups[tr].tolist()), set(groups[va].tolist())
    overlap = g_tr & g_va
...
if CFG["PSEUDO_DIR"] is not None:
    p = Path(CFG["PSEUDO_DIR"]) / "pseudo_labels.parquet"
    if p.exists():
        pseudo_df = pd.read_parquet(p)
        print(f"Loaded pseudo labels: {len(pseudo_df)} rows from {p}")
