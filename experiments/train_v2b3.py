...
class BirdSED(nn.Module):
    def __init__(self, backbone_name, num_classes=234, drop_path_rate=0.15, specaug_enabled=True,
        ...
        self.backbone = timm.create_model(
            backbone_name, pretrained=imagenet_pretrained, in_chans=1,
...
def main():
    ...
    ap.add_argument('--backbone', default='tf_efficientnetv2_b3.in21k_ft_in1k')
    ap.add_argument('--pretrained', default='imagenet',
                    help="Path to a bird-pretrained .ckpt, or 'imagenet'/'none' to use timm ImageNet init")
    ap.add_argument('--drop_path_rate', type=float, default=0.15)
    ap.add_argument('--out_dir', default=str(WORK / 'sed_v2b3_e21_out'))
    ap.add_argument('--pseudo_parquet', default=str(WORK / 'pseudo_labels_v2.parquet'))
    ...
    _use_imagenet = str(args.pretrained).lower() in ('imagenet', 'timm', 'none', '')
    model = BirdSED(args.backbone, num_classes=234, drop_path_rate=args.drop_path_rate,
                    specaug_enabled=(not args.no_specaug),
                    imagenet_pretrained=_use_imagenet).to(device)
    if _use_imagenet:
        print(f'Pretrained: timm ImageNet weights (in_chans=1 adapted), drop_path={args.drop_path_rate}')
    else:
