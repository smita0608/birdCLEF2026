...
class SpecAugment(nn.Module):
    def __init__(self,
                 freq_mask_max=10, freq_mask_n=3, freq_p=0.3,
    ...
    def forward(self, mel):
        if not self.training or self.freq_mask_max <= 0:
            return mel
        B, C, Fdim, Tdim = mel.shape
        for _ in range(self.freq_mask_n):
            if random.random() < self.freq_p:
                w = random.randint(1, self.freq_mask_max)
                if w >= Fdim:
                    continue
                f0 = random.randint(0, Fdim - w)
                mel[:, :, f0:f0 + w, :] = 0.0
        for _ in range(self.time_mask_n):
            if random.random() < self.time_p:
                w = random.randint(1, self.time_mask_max)
                if w >= Tdim:
                    continue
                t0 = random.randint(0, Tdim - w)
                mel[:, :, :, t0:t0 + w] = 0.0
        return mel
...
class FocalLossBCE(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        return self.bce_weight * bce_loss + self.focal_weight * focal
...
PSEUDO_TRIM_MIN_PROB = 0.1
PSEUDO_PRIMARY_MIN_PROB = 0.5
PSEUDO_SAMPLING_PROB = 0.4


class FocalPseudoDataset(Dataset):
    def __init__(self, focal_df, train_dir, class2idx,
        ...
        self.idx2class = {i: c for c, i in class2idx.items()}
...
def main():
    ...
    ap.add_argument('--out_dir', default=str(WORK / 'sed_v2s_sydorskyy_out'))
    ap.add_argument('--pseudo_parquet', default=str(WORK / 'pseudo_labels_sydorskyy.parquet'))
    ap.add_argument('--pseudo_sampling_prob', type=float, default=PSEUDO_SAMPLING_PROB)
    ap.add_argument('--pseudo_trim_min_prob', type=float, default=PSEUDO_TRIM_MIN_PROB)
    ap.add_argument('--pseudo_primary_min_prob', type=float, default=PSEUDO_PRIMARY_MIN_PROB)
    ap.add_argument('--label_smoothing', type=float, default=LABEL_SMOOTHING)
    ap.add_argument('--mixup_p', type=float, default=0.5)
    ap.add_argument('--no_pseudo', action='store_true', help='Disable pseudo replacement (ablation)')
    ap.add_argument('--no_specaug', action='store_true', help='Disable SpecAugment (ablation)')
    ...
    if args.smoke:
        ...
        print('SMOKE MODE: 3 epochs, 50 focal samples/class max, full pseudo set')
    ...
    print(f'Building model: {args.backbone}  drop_path=0.15  specaug={not args.no_specaug}')
    ...
    print(f'Pretrained load: missing total={len(missing)}, '
          f'head_missing={len(head_missing)}, backbone_missing={len(backbone_missing)}')
    ...
    if not args.no_pseudo:
        ...
        print(f'Pseudo rows: {len(pseudo_df)} (primary_prob > {args.pseudo_primary_min_prob}), '
              f'covering {n_classes_with_pseudo}/{len(classes)} classes')
    else:
        print('Pseudo replacement DISABLED via --no_pseudo')

    ds = FocalPseudoDataset(
        focal_df=train_csv, train_dir=BASE / 'train_audio', class2idx=class2idx,
        pseudo_df=pseudo_df,
        pseudo_sampling_prob=args.pseudo_sampling_prob,
        trim_min_prob=args.pseudo_trim_min_prob,
    ...
    print(f'Optim: AdamW lr={args.lr}  Schedule: CosineAnnealingWarmRestarts T_0={total_steps} eta_min=1e-6')
    print(f'Loss:  FocalLossBCE(α=0.25, γ=2, bce_w=1, focal_w=1)')
    print(f'AMP:   {use_amp}')
    ...
    for ep in range(args.epochs):
        ...
        oof_auc, preds = eval_soundscape_oof(model, meta, device, use_amp=use_amp)
    ...
    if final >= 0.85:
        print('  ✓ STRONG — full integration likely worthwhile')
    elif final >= 0.80:
        print('  ~ PROMISING — integration cautiously worthwhile')
    elif final >= 0.75:
        print('  ~ MARGINAL — about the same as e11 (no clear lift from Sydorskyy recipe)')
    elif final >= 0.65:
        print('  ~ WEAK — domain gap not fully bridged')
    else:
        print('  ✗ COLLAPSE — something is wrong, do not run full training')
